void digi_complete()
{
  ...
  // -----   START digi producers   ---------------------------------
  PndStartDigiTask* startdigi = new PndStartDigiTask();
  startdigi->SetVerbose(iVerbose);
  fRun->AddTask(startdigi);
	
  PndStartClusterTask* startmccls = new PndStartClusterTask();
  startmccls->SetVerbose(iVerbose);
  fRun->AddTask(startmccls);
