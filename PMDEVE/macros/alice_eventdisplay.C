...
{
   Info("alice_eventdisplay.C",
        "Has to be run in compiled mode ... doing this for you.");
   gSystem->CompileMacro("alice_eventdisplay.C");
   alice_eventdisplay();
}
#else
...
#include "/home/shyam/alice/root/alice_v5-34-30/src/tutorials/eve/MultiView.C"
...
void alice_eventdisplay(const char* vsd_file_name=
               "./AliVSD.root")
{
   ...
   { // Simple geometry
      TFile* geom =
      TFile::Open("./simple_geom_All.root",
                  "CACHEREAD");
      if (!geom)
         return;
      TEveGeoShapeExtract* gse = (TEveGeoShapeExtract*) geom->Get("All");
